...
const K3_RISCV_MUSL_TARGET: &str = "riscv64gc-unknown-linux-musl";
const K3_ORT_LIB_PATH: &str = "K3_ORT_LIB_PATH";

fn system_lib_path() -> Option<String> {
	let target = env::var("TARGET").expect("Cargo must set TARGET for build scripts");

	if target == K3_RISCV_MUSL_TARGET {
		// Cargo's [env] configuration is global, not target-scoped.  Keep the
		// K3 library path separate so x86_64 host builds can use ort-sys's
		// normal prebuilt-runtime path instead of RISC-V static archives.
		return vars::get(K3_ORT_LIB_PATH).or_else(|| vars::get_any(vars::SYSTEM_LIB_PATH));
	}

	vars::get_any(vars::SYSTEM_LIB_PATH)
}
...
fn main() {
	...
	if let Some(lib_dir) = system_lib_path() {
