...
pub fn static_link(base_lib_dir: &Path) -> bool {
	...
	'main: for (lib_dir, extension_lib_dir, external_lib_dir, transform_dep) in static_configs {
		if lib_dir.join(platform_format_lib("onnxruntime_common")).exists() {
			...
			// Non-minimal ORT builds split Model Package support into this archive.
			// It is a dependency of onnxruntime_session in a fully static link.
			...
			let cpuinfo_build_dir = transform_dep(external_lib_dir.join("pytorch_cpuinfo-build"), &profile);
			if !has_vcpkg_link {
			...
			optional_link_lib(&cpuinfo_build_dir, "cpuinfo");
			...
			if optional_link_lib(&lib_dir, "onnxruntime_providers_k3") {
				// The in-tree K3 EP builds its Rust runtime under the ORT CMake build
				// directory. Its target triple matches Cargo's final-link target.
				let k3_runtime_dir = lib_dir
					.join("k3-cargo")
					.join(env::var("TARGET").unwrap())
					.join("release");
				optional_link_lib(&k3_runtime_dir, "k3_ai_runtime");
