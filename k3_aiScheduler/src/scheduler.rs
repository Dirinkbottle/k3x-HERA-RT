...
/// 内核提交入口
/// 'k_graph' 为内核vddr
pub fn run_graph(user_token: u32, caller: Box<dyn K3SchedulerOps>, complete_sender: Sender<'static>, tasklink: TaskLink) -> Result<(), SchedulerErr> {
    ...
    loop {
        ...
        if let Some(unit) = unit {
            ...
            for node in unit.tasklink.iter() {
                ...
                if ret != 0 {
                    error!("k3_run_kernel failed: node_id={}, op={:?}, ret={}", node.node_id, node.desc.op, ret);
                    success = false;
                    break;
                }
                error!("k3_run_kernel success: node_id={}, op={:?}", node.node_id, node.desc.op);
            }
            ...
            if success {
                // 通知调用者完成,回写token
                if let Ok(()) = unit.complete_sender.try_send(&Message::notification(unit.user_token)) {
