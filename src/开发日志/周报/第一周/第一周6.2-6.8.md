...
其次开始写用户态算子库 frontend 的最小结构。

目前 frontend 里先拆出了几类基础结构：

- `KernelOp` 和 `AiTargetHint`：描述这是哪个语义级算子，以及用户态对 CPU/X100/A100 的倾向。最终跑在哪个核心仍然交给调度器决定。
- kernel attr：把 MatMul、RMSNorm、RoPE、Softmax、Conv2d 等算子的参数做成固定大小的 `repr(C)` 结构，后续可以直接内联进提交描述。
- `AiSubmitEntry`：作为提交到 uring 的最小 kernel desc。它里面只放稳定可复制的数据，包括 op、target hint、user token、输入输出 tensor 描述和内联 attr。
- `AiTensorDesc`：作为可传递的 tensor 描述，只描述用户态 buffer 地址、大小、dtype、format、layout、shape 等信息，本身不拥有内存。
- `Tensor`、`TensorManager` 和 `TensorDataToken`：`Tensor` 只作为用户态数据实体，由 manager 管理生命周期；`TensorDataToken` 用来在用户态库内部重新找到对应数据；真正传给内核的是 `AiTensorDesc`，也就是 desc 指向被 manager 管理的 tensor 数据。
